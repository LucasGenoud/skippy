...
async fn auto_labeling_applies_only_existing_labels() {
    ...
    create_note(
        &app,
        &token,
        json!({"title": "Pasta", "content": "tomato, basil"}),
    )
    .await;
    ...
    assert!(
        prompt.contains("work") && prompt.contains("recipes"),
        "{prompt}"
    );
...
async fn auto_labeling_is_add_only() {
    ...
    assert!(
        ids.contains(&json!(work_id)) && ids.contains(&json!(recipes_id)),
        "{ids:?}"
    );
}
...
async fn auto_labeling_skipped_when_unconfigured_off_or_labelless() {
    ...
    assert_eq!(
        calls.lock().unwrap().len(),
        0,
        "toggle off must not call the LLM"
    );
    ...
    assert_eq!(
        calls.lock().unwrap().len(),
        0,
        "no labels means nothing to assign"
    );
}
...
async fn note_rewrite_requires_opt_in_and_updates_content() {
    ...
    let note = create_note(
        &app,
        &token,
        json!({"title": "Long title", "content": "bad sentence"}),
    )
    .await;
    let id = note["id"].as_str().unwrap();
...
async fn a_managed_toggle_disables_the_feature_for_the_user() {
    ...
    assert!(
        ...
        "labeling ran despite being pinned off"
    );
    let note = &list_notes(&app, &token).await[0];
...
async fn managed_settings_endpoint_lists_keys_and_redacts_secrets() {
    ...
    assert_eq!(
        body["llm_base_url"],
        json!({"secret": false, "value": "http://managed/v1"})
    );
