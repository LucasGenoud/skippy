...
async fn attachment_upload_serve_delete() {
    ...
    let signed_url = attachment["url"]
        .as_str()
        .expect("signed url in upload response")
        .to_string();
...
async fn file_urls_require_valid_signature() {
    ...
    // No signature.
    assert_eq!(
        status_of(&app, format!("/api/files/{att_id}")).await,
    ...
    assert_eq!(
        status_of(
            &app,
            format!("/api/files/{att_id}?exp={}&sig={good}", future + 999_999)
        )
        .await,
        StatusCode::UNAUTHORIZED
    );
    ...
    assert_eq!(
        status_of(
            &app,
            format!("/api/files/{att_id}?exp={past}&sig={expired}")
        )
        .await,
        StatusCode::UNAUTHORIZED
    );
    ...
    assert_eq!(
        status_of(
            &app,
            format!("/api/files/{att_id}?exp={future}&sig={other}")
        )
        .await,
        StatusCode::UNAUTHORIZED
    );
