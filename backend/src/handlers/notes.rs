...
pub async fn create_note_for_user(
    ...
    state.summarize_links_later(&id, user_id, note_urls(&record));
...
pub async fn apply_note_update(
    ...
    apply_note_update_inner(state, user_id, id, body, true).await
}

/// Apply content generated by an automatic summary without treating any URLs
/// in the model reply as newly user-added links.
pub(super) async fn apply_auto_summary_update(
    state: &AppState,
    user_id: &str,
    id: &str,
    body: UpdateNote,
) -> ApiResult<NoteView> {
    apply_note_update_inner(state, user_id, id, body, false).await
}

async fn apply_note_update_inner(
    state: &AppState,
    user_id: &str,
    id: &str,
    mut body: UpdateNote,
    ...
    if schedule_summaries {
        let added_urls: Vec<String> = note_urls(&record)
            .into_iter()
            .filter(|url| !urls_before.contains(url))
            .collect();
        state.summarize_links_later(id, user_id, added_urls);
    }
...
fn note_urls(record: &NoteRecord) -> HashSet<String> {
